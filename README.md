...
## OmniMesh AI backend (AMD Hackathon ACT II)

A **parallel cloud-grade reasoning layer** lives in [`backend/`](backend/) — a FastAPI multi-agent service built for the **AMD Developer Hackathon: ACT II (Unicorn Track)**. It does **not** replace the existing Gemini / Firebase dispatch path; both coexist:

| Path | When to use | Stack |
|------|-------------|-------|
| **Gemini (existing)** | Default offline-native fallback, Firebase Functions, legacy dashboard flow | Google Gemini API |
| **OmniMesh AI backend** | Optional “cloud-grade” triage when online — **hybrid local/cloud routing demo centerpiece** | **Gemma via Fireworks AI** on AMD infrastructure, edge heuristics offline |
...
- `GET /health` — liveness + current connectivity flag
- `POST /v1/triage` — orchestrated triage (`local` / `cloud` / `hybrid`)
- `POST /v1/connectivity/on` · `POST /v1/connectivity/off` — live demo uplink toggle
...
Start the React app (`npm start --prefix web`), open **Command**, switch **Dispatch engine → OmniMesh AI**, and use the header **Connectivity** toggle to flip offline/online routing during the demo. The fired mode (`LOCAL` / `CLOUD` / `HYBRID`) renders as a prominent badge — this is the judging centerpiece.
