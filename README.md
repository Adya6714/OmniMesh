...
<p align="center">
  <a href="https://www.amd.com/en/developer.html" title="AMD Developer Cloud"><img src="https://img.shields.io/badge/AMD_Developer_Cloud-000000?style=for-the-badge&logo=amd&logoColor=white" alt="AMD Developer Cloud" /></a>
...
## OmniMesh AI backend (AMD Hackathon ACT II — Unicorn Track)

A **parallel cloud-grade reasoning layer** lives in [`backend/`](backend/) — a FastAPI multi-agent service built for the **AMD Developer Hackathon: ACT II**. It does **not** replace the existing Gemini/Firebase dispatch path; both coexist, switchable live from the dashboard.

**Gemma runs on AMD hardware two independent ways** — the centerpiece of this submission:

| Path | Where Gemma runs | When it fires |
|------|------|------|
| **Local (edge)** | **Gemma-2-9B, served on our own AMD GPU via vLLM-ROCm** on AMD Developer Cloud | Offline, or as the instant first answer in hybrid mode |
...
- **Hybrid routing orchestrator** — decides per-packet whether to answer locally (edge GPU), in the cloud, or both, and logs a human-readable `reasoning_trace` for every decision (explainability, not a black box).
- **Uncertainty-aware triage** — every answer reports a confidence/uncertainty score and flags low-confidence cases with `needs_human_review`, so the system never silently guesses on a life-critical call.
- **Vision damage assessment** — structural damage severity from a photo, via a multimodal model on Fireworks.
- **Missing-person matching** — semantic matching of missing-person reports against triaged victims, with an LLM explaining *why* a match was made; falls back to offline token-similarity with zero connectivity.
- **Live AMD GPU proof** — `GET /v1/gpu-status` returns real-time confirmation that Gemma is loaded and serving on the AMD GPU, inspectable at any moment during judging, not just a screenshot.
...
The API listens on **http://localhost:8000** (or `:8080` if remapped) with:
...
17+ unit tests, all passing (`cd backend && python -m pytest tests -v`). Full testing walkthrough in [`backend/MASTER_TEST_PLAN.md`](backend/MASTER_TEST_PLAN.md).

### AMD platform evidence

Real ROCm-specific behavior observed in production logs on our AMD Developer Cloud instance (not generic fallback paths):
...
- `Maximum concurrency for 4,096 tokens per request: 115.66x` — real headroom for concurrent triage requests at scale
...
```
Open **Command**, switch **Dispatch engine → OmniMesh AI**, and use the header **Connectivity** toggle to flip offline/online routing live. The fired mode (`LOCAL` / `CLOUD` / `HYBRID`) renders as a badge — this is the judging centerpiece.
...
Use **`make setup`** or **`npm run setup`** at the repo root, then follow **[docs/HACKATHON_SETUP.md](docs/HACKATHON_SETUP.md)** for Firebase, Android USB debugging, and mesh testing on real hardware.
...
- **Gemini Flash HTTP vision** (`VisionClassifier`) rather than a bundled EfficientNet TFLite — convenient for hackathon iteration & genuine semantic captions.
...
<p align="center"><sub><strong>OmniMesh technical README</strong> · structured urgency over unstructured chatter · maintained for hackathon disclosure</sub></p>
